...
var _ = FDescribe("ClusterScopedResourceClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: ClusterScopedResourceCrd},
	} {
		Context("resource client backed by "+test.Description(), func() {
			var (
				client ClusterScopedResourceClient
				err    error
			)
			BeforeEach(func() {
				namespace = helpers.RandString(6)
				factory := test.Setup(namespace)
				client, err = NewClusterScopedResourceClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs ClusterScopedResources", func() {
				ClusterScopedResourceClientTest(namespace, client)
			})
		})
	}
})

func ClusterScopedResourceClientTest(namespace string, client ClusterScopedResourceClient) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := "foo"
	input := NewClusterScopedResource(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&ClusterScopedResource{}))
